...
class QueueCommandString
{
    ...
    {
        return sprintf('--workers-name=%s --balance=%s --max-processes=%s --min-processes=%s --nice=%s --balance-cooldown=%s --balance-max-shift=%s --balance-max-scale-down=%s --parent-id=%s --auto-scaling-strategy=%s %s',
            $options->workersName,
            $options->balance,
